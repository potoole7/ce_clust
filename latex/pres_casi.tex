...
\begin{frame}{Asymptotic dependence}
  \begin{itemize}
    ...
    \item However, $\chi$ only gives summary; \\
      inference requires \textbf{dependence model}.
  \end{itemize}
\end{frame}
...
\begin{frame}{Marginal transformation}
  ...
  % dependence strength
  \center
  \includegraphics[width=1\textwidth]{plots/transform_costelloe.png} 
\end{frame}
...
\begin{frame}{skew-geometric Jensen-Shannon divergence}
  \begin{itemize}
    \item <1-> Kullback-Leibler divergence $KL(P, Q)$ measures (asymmetric) distance from $P$ to $Q$
    \item <1-> $KL(P, Q)$ has closed form for two MVNs
