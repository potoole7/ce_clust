...
From equation \ref{eq:piecewise_marg}, we can estimate the probability of $X_i$ exceeding the threshold $u_{X_i}$, handling the second part of equation \ref{eq:joint_prob}. \todo{Fix this reference}
...
% , and thus there is no need for the indicator function, so \ref{eq:gumbel_normalisation} simplifies to
...
Finally, we now nonparametrically estimate $G_{\mid i}$ using the empirical distribution of 
\[
