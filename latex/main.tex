...
\begin{itemize}
  \item We begin with a short exploration of the data, looking at the $\chi$ and $\bar{\chi}$ statistics introduced in section \ref{subsec:asym_dep}.
  \item Specifically, we looked at their values at the 95th quantile for each of our 59 sites, to determine the extent of asymptotic dependence between our variables, and whether any spatial patterns emerge. 
  \item We can see that the $\chi$ and $\bar{\chi}$ values are positive for almost all sites, indicating positive asymptotic dependence of varying degree between rain and wind speed.
  \item A clear spatial pattern emerges, whereby extremal dependence is progressively stronger for locations further south and west, being highest along the west coast of Ireland, in counties Galway, Mayo and Kerry. \todo{What about Donegal?}
...
\begin{itemize}
  \item In our analysis, we first transformed the data at each location to Laplace margins, using the ECDF. \todo{reference specific equation in methods}
    \item We then fit the CE model to the data at each location.
    \item By performing bootstrapping the values of $\alpha$ and $\beta$ for each location at various dependence quantiles, we can see that the estimates in general are quite stable up to the 88th quantile. Subsequent analysis was also performed at the 85th and 90th quantiles, and shown to provide similar MLEs for our CE parameters, and clustering solutions. \todo{Need to expand with more details?}
    \item The results of this CE analysis are shown in figure \ref{fig:02_locs}.
    \item Interpretation of these plots is difficult, but we can see some, albeit uneven, increases in the $\alpha$ values for both variables as we move further West and particularly South. \todo{Should I estimate/show uncertainty in these estimates somehow? I'\ve bootstrapped these estimates!}
    \item Several $\beta$ values are negative, which is unexpected, and perhaps suggests non-identifiability in the CE model fits. 
...
\begin{itemize}
  \item We clustered the locations using the $JS^{G_{\alpha}}$ dissimilarity matrix for their CE parameters, as described in \ref{subsec:clust_js}, for the models for rain conditional on wind speed and vice versa, together, by summing their dissimilarity matrices, and separately.
  ...
  For all three clustering solutions, clear elbows were observed at $k = 3$, so this was the number of clusters chosen \todo{Include plots? Important/interesting?}.
  \item The clustering solutions for the 88th dependence quantile for each of our three dissimilarity matrices is shown in figure \ref{fig:clust_sol}.
  \item The solutions are \todo{Return here!}
\end{itemize}
