...
\begin{frame}{Selecting $k$ with total within cluster sum of squares \& elbow plot}
  \begin{itemize}
    \item \textbf{TWGSS}:
      \[
        \mathrm{TWGSS}(k)
        = \sum_{i=1}^{k}\sum_{x \in C_i} d\bigl(x, m_i\bigr)
      \]
...
\begin{frame}{$\alpha, \beta$ estimates}
  \includegraphics[width=1\textwidth]{plots/ire_ce.png} 
\end{frame}

% Maps of clustering
\begin{frame}{Clustering Result}
  TODO: Include clustering results for both variables and just rain \& windspeed
\end{frame}
