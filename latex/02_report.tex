...
Define the upper end-point of F as $z_+ = \sup(F)$. \todo{Define as z up F as in Shooter2020 so we can use + below for GPD}
...
The Fisher-Tippet-Gnedenko theorem (\cite{Fisher1928}, \cite{Gnedenko1943}) states that the limiting distribution of these non-degenerate maxima,
...
is in the max-domain of attraction, or less formally converges to, the Generalised Extreme Value (GEV) distribution. \todo{word better}
...
\todo{Mathematical motivation/argument for mean residual life plot}
\todo{Less detail in places}
...
Two commonly used diagnostic tools to aid in the choice of $u$ are the so-called mean residual life and threshold stability plots \cite{Coles2001}.
...
This method is useful when we want to ``semi-automatically'' estimate thresholds across different times and/or spatial locations, which is addressed in greater detail in \ref{subsubsec:non_stationary}.
