...
\begin{figure}[H]
    \centering
    \includegraphics[width = 0.9\linewidth]{plots/044_density_alpha.png}
    \caption{\emph{Test}}
...
\begin{enumerate}
  ...
  \item Showed how univariate extremes can be modelled using GPD, parameter estimates can include covariates, and how GPD can be applied to Ireland dataset. 
  \item Introduced dependence modelling and the conditional extremes model with some previous applications, why its an improvement over previous models (brief), some extensions, an application which shows where it has some limitations, being susceptible to uncertainty in $\xi$ estimates which propogate into $\alpha$ and $\beta$ (some limitations to this approach, fixed $\beta$, calculated CE model for each location separately (although GPD modelled spatially))
  \item Reviewed clustering methods for extremes, examples of both types of clustering (explanatory and hierarchical), highlighted some papers where there is potential to use these methods on the conditional extremes model.
\end{enumerate}
