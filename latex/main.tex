...
%       KL(P \mid\mid Q) = \frac{1}{2} \left( \frac{(\mu_2 - \mu_1)^2}{\sigma_2^2} + \frac{\sigma_1^2}{\sigma_2^2} - \log{(\frac{\sigma_1^2}{\sigma_2^2})} - 1 \right),
%     \end{equation}
...
%     \end{equation}
%     where $M = \frac{1}{2}(P + Q)$ \citep{Lin1991}. \todo{IMPORTANT: Check with Christian if this is a mixture distribution or an ``average'' of the two, if mixture then JS is not closed!}
%     \todo{May have to use the geometric JS divergence instead! Everything included in https://pmc.ncbi.nlm.nih.gov/articles/PMC7514974/#sec4dot1dot2-entropy-21-00485}
...
\todo{skew-geometric JS divergence is unbounded, need to include Thiagarajan version which is!}
