...
\begin{frame}{Steps}
  \begin{enumerate}
    ...
    \item <3-> Calculate $N\left(\bm{\alpha} y_i + y_i^{\bm{\beta}} \bm{\mu}, y_i^{\bm{\beta}}\Sigma \right)$
    \begin{itemize}
      \item <4-> $y_i \sim \text{Exponential}(1) + u_{Y}$
      ...
      % \end{align*}
    \end{itemize}
    \item <5-> Calculate pairwise $JS^{G_{\alpha}}$ dissimilarity between sites for each variable
    \item <5-> Cluster!
\end{enumerate}
\end{frame}
