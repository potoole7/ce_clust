...
The Pickands-Balkema-de-Haan theorem (\cite{Pickands1975, Balkema1974}) states that should $M_n$ satisfy equation \eqref{eq:uni_limiting_dist} and converge in distribution to the GEV distribution, then for any $Z = Z_i, 1 \le i \le n$ in our sequence, the distribution of exceedances $z > 0$ over a high threshold $u$ are approximated by the Generalised Pareto Distribution (GPD), defined as
...
\end{equation}
where $\hat{H}(z)$ is the cumulative distribution function (CDF) of the GPD, equivalent to 1 - equation \eqref{eq:gpd}, for the corresponding parameter estimates $\hat{\sigma}$ and $\hat{\xi}$. \todo{Need to spell out with Hz is?}
...
\]
ensuring that there is no mass at $+\infty$ for any margin, but allowing for some mass to be at $-\infty$ \citep{Keef2013}. \todo{Why is this}
