...
# TODO: Try smooth shape also? Probably not required
...
ggsave(
  "plots/gpd_rain_ire_var_thresh.png", p_scale_rain, width = 6.3, height = 6, units = "in"
)
...
ggsave(
  "plots/gpd_ws_ire_var_thresh.png", p_scale_ws, width = 6.3, height = 6, units = "in"
)
...
# saveRDS(marginal, file = "marginal.RDS")
...
# saveRDS(dependence, file = "dependence.RDS")
...
# sf::write_sf(a_b_sf, "a_b_sf.geojson")
...
ggsave("plots/a_b_ce_ire_var_thresh.png", p_lst[[1]] / p_lst[[2]], width = 16, height = 12, units = "in")
...
# diagnostic plots for rain and windspeed, respectively
pdf("plots/diag_plots_rain_var_thresh.pdf", onefile = TRUE) 
for (i in seq(length(plots))) {
  do.call("grid.arrange", plots[[i]][[1]])
}
dev.off()

pdf("plots/diag_plots_windspeed_var_thresh.pdf", onefile = TRUE) 
for (i in seq(length(plots))) {
