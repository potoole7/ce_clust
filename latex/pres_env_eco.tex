...
\begin{frame}{skew-geometric Jensen-Shannon divergence}
  \begin{itemize}
    \item <1-> Kullback-Leibler divergence $KL(X \mid\mid Y)$ measures (\textbf{asymmetric}) distance from $X$ to $Y$
    % triangle inequality: “direct” distance between two distributions is never larger than going through a third.
    \item <1-> $KL(X \mid \mid Y)$ has closed form for two MVNs
    \item <1-> $KL(X \mid\mid Y) + KL(Y \mid\mid X)$ symmetric but does not satisfy triangle inequality $\implies$ \textbf{not a true metric}.
    \item <2-> Weighted geometric mean $G_{\alpha}(X, Y) = X^{\alpha} Y^{1 - \alpha}, \alpha \in [0, 1]$
    ...
    \item <4-> \textbf{Skew-geometric Jensen-Shannon divergence}:
      \[
        JS^{G_{\alpha}}(X \mid\mid Y) = \frac{1}{2} \left\{KL(X \mid\mid G_{\alpha}(X, Y)) + KL(Y \mid\mid G_{\alpha}(X, Y))\right\}
      \]
  \end{itemize}
  \vspace{0.5cm}
  {\only<4>{\scriptsize
...
\begin{frame}{Gaussian copula}
  ...
  \begin{itemize}
    ...
    \item 3 clusters of 4 locations defined by respective $\rho_{\text{Gauss}}$ values of $0.3, 0.6, 0.9$.
  \end{itemize}
\end{frame}
