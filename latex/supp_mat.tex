...
%   % Only executes when compiling supplement directly
%   \documentclass[12pt,compact]{article}
% \fi
...
\if0\blind
{
  ...
  \title{\bf Supplementary materials for ``Clustering of multivariate tail dependence using conditional methods''}
  \author{%
...
% \bigskip
\begin{abstract}
  % This supplementary materials contains additional plots for the motivating data application in Section 4 of the main paper, including elbow plots for choosing the number of clusters, clustering solutions for $k=2$ and $k=4$ clusters, threshold stability plots, and model diagnostic plots for the conditional extremes model fits at each location.
  % This supplementary materials contains additional plots for the motivating data application in Section 4 of the main paper, including elbow plots for choosing the number of clusters, clustering solutions for $k=2$ and $k=4$ clusters, and threshold stability plots for the conditional extremes model fits at each location.
  This supplementary materials contains additional plots for the motivating data application in Section~\ref{sec:application} of the main paper, including elbow plots for choosing the number of clusters, clustering solutions for $k=2$ and $k=4$ clusters, and threshold stability plots for the conditional extremes model fits at each location.
\end{abstract}
...
Figure~\ref{fig:elbow_plots} gives the ``elbow'' plots of the total within-group sum of squares (TWGSS) against the number of clusters $k$, as described in Section~\ref{subsec:clustering}, for the motivating data application in Section~\ref{sec:application}, both in the main paper.
These are shown for the clustering solutions obtained using the dissimilarity matrix for wind speed conditional on precipitation, precipitation conditional on wind speed, and the aggregated dissimilarity matrix $M$ as described in Equation~\eqref{eq:aggregate_matrix} of the main paper.

\begin{figure}[ht]
    \centering
    \includegraphics[width = 0.9\linewidth]{plots/cluster_dqu_sep_elbow.png}
    \caption{\emph{ 
      ``Elbow'' plots of the total within-group sum of squares (TWGSS) against the number of clusters $k$ for the clustering solutions obtained using the dissimilarity matrix for wind speed conditional on precipitation (centre), precipitation conditional on wind speed (right), and the combined dissimilarity matrix (left).
    }}
...
Figure~\ref{fig:cluster_dqu_k_2_4} shows the clustering solutions for $k=2$ and $k=4$ clusters obtained using the aggregated dissimilarity matrix $M$, as mentioned in Section~\ref{subsec:app_clust} of the main paper.
...
Here, we present threshold stability plots for the conditional extremes model fits at the locations highlighted in Figure~\ref{fig:chi} of the main paper, or specifically, Malahide Castle and Ringsend, both in county Dublin, Kilcar in county Donegal, and Derryhillagh in county Mayo.
