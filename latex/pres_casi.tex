...
\begin{frame}{Asymptotic dependence}
  \begin{columns}[t]
    % Left column: with an itemize
    \begin{column}[t]{0.48\textwidth}
      \vspace{0pt}          % top‐anchor
      \small                % shrink font locally
      \begin{itemize}
          \item Coefficient of extremal dependence $\chi \in [0, 1]$
          \todo{Reduce space between ``where'' and equation}
          \todo{Introduce data after this slide?}
          \item $\chi = \lim_{u \rightarrow 1}{\chi(u)}$ where
          \adjustbox{valign=t}{%
...
\begin{frame}{Diagnostic Plots}
  \todo{Add diagnostic plots for a given location}
  \includegraphics[width=1\textwidth]{plots/transform.png} 
\end{frame}
...
\begin{frame}{skew-geometric Jensen-Shannon divergence}
\begin{itemize}
  \item Kullback-Leibler divergence $KL(P, Q)$ measures (asymmetric) distance from $P$ to $Q$.
  \item Weighted geometric mean $G_{\alpha}(x, y) = x^{\alpha} y^{1 - \alpha}, \alpha \in [0, 1]$
  % \item The weighted product of two members of the same exponential family also comes from this family, so we will have another multivariate normal. 
  \item Using $G_{\alpha}$, we get \textbf{skew-geometric Jensen-Shannon divergence}
    \[
      JS^{G_{\alpha}}(P \mid\mid Q) = \frac{1}{2} KL(P \mid\mid G_{\alpha}(P, Q)) + \frac{1}{2} KL(Q \mid\mid G_{\alpha}(P, Q))
    \]
  % \item Has both closed form for MVNs and is true metric, making it a good candidate as a dissimilarity measure for clustering. 
  \item weighted product of two members of same exponential family comes from \textbf{same family}.
  \item Therefore, for MVN distributions, $JS^{G_{\alpha}}$ has closed form.
  % \item Has both closed form for MVNs and is true metric $\implies$ found suitable dissimilarity measure!
  % \item Sensible choice is $\alpha = 0.5$. %, may adapt data-driven or theoretically motivated approach in the future. 
\end{itemize}
\end{frame}
...
\begin{frame}{Clustering}
  \begin{itemize}
    \item Uses \textbf{k-medoids} to cluster over $JS_{G_{\alpha}}$ dissimilarity matrix
...
% Maps of clustering
\begin{frame}{Ireland clustering result}
