...
For example, we may be interested in modelling the joint probability of extreme rainfall and wind speed at a given location, which may together produce an especially damaging storm event.
...
Similarly, we may also be interested in the joint probability of extreme rainfall at different locations, which may lead to widespread flooding for which flood defences need to be prepared.
Furthermore, in \cite{Zhang2024}, return levels have been shown to be improved by modelling dependence between extremes.
...
The second of these probabilities can be estimated using a univariate GPD as in section \ref{subsec:uni}, but the first requires a model for the dependence structure between the components of $\bm{X}$.

In this section, we will first introduce the concept of dependence modelling for extremes more generally, and then describe the conditional extremes model, a flexible model for extremal dependence, and how it can be fit to data and subsequently interpreted.
...
\]
Under asymptotic independence, $\bar{\chi} = 0$, while for asymptotic dependence $\bar{\chi} = 1$.  \todo{Rewrite to not be the same as the following line}
Under asymptotic independence, $\bar{\chi}$ increases in magnitude as the variables become more positively (for $\bar{\chi} > 0$) or negatively (for $\bar{\chi} < 0$) dependent in their extremes \cite{Vignotto2021}.

Together, $(\chi, \bar{\chi})$ describe the extremal dependence of any pair of random variables.
However, while they provide a useful measure of extremal dependence, they do not provide a full description of the dependence structure of a multivariate distribution.
For example, we cannot calculate the joint probability of observing extreme values in more than two variables using these coefficients.
% For this, we require a model for extremal dependence, such as the conditional extremes model.
...
Max-stable processes are also quite computationally expensive, and so are often not feasible for large datasets.
\todo{Check this in Tawn 2018 and H\&T 2004}
...
The conditional extremes model combines a piecewise semi-parametric marginal extremes model which includes a fitted GPD as described in \ref{sec:uni} with a semi-parametric regression model for the dependence structure. 
...
The severe heatwave experienced by Australia in 2009 was modelled in \citep{Winter2016} to understand the relationship between the El Niño-Southern oscillation and extreme temperature.
\todo{Mention application in Keef 2012?}
\todo{More recent references? Brain waves?}
...
\todo{Return levels through simulations, refer to Keef2012_flooding}
