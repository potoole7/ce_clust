...
\begin{frame}{KL Divergence}
  \begin{itemize}
    \item <1-> We have CE models $Y \mid X$ across all sites
    \item <2-> A popular form of statistical distance is the Kullback-Leibler divergence % measures (\textbf{asymmetric}) distance from models $P$ to $Q$
    \[
    ...
    \item <3-> $KL(P \mid \mid Q)$ has closed form for two MVNs
    \item <4-> Why not $KL(P \mid\mid Q) + KL(Q \mid\mid P)$? % symmetric but does not satisfy triangle inequality $\implies$ \textbf{not a true metric}.
