...
\begin{frame}{KL Divergence}
  \begin{itemize}
    \item <1-> Kullback-Leibler divergence $KL(P \mid\mid Q)$ measures (\textbf{asymmetric}) distance from $P$ to $Q$
...
\begin{frame}{Skew-geometric Jensen-Shannon divergence}
  \begin{itemize}
    ...
    \item <4-> P, Q depend on X $\implies$ we define the expected value
      ...
      \]
      where $f(x)$ is the marginal density of $X \mid X > u_X$.
    \item <5-> $\implies$ Can use MC integration to estimate $JS^{G_{\alpha}}$ between MVNs at different sites.
  \end{itemize}
  \vspace{0.5cm}
  {\only<4>{\scriptsize
    \rule{\linewidth}{0.4pt}\\
...
\begin{frame}{Steps}
  \begin{enumerate}
    ...
    \item <3-> Calculate $N\left(\bm{\alpha} y_i + y_i^{\bm{\beta}} \bm{\mu}, y_i^{\bm{\beta}}\Sigma \right)$ at each site.
    \begin{itemize}
      \item <4-> Since Laplace has exponential tails, $y_i \sim \text{Exponential}(1) + \max{(u_{Y_i})}$
      ...
      % \end{align*}
    \end{itemize}
    \item <5-> Calculate pairwise $JS^{G_{\alpha}}$ dissimilarity matrices across all sites for each variable
    \item <5-> Cluster!
\end{enumerate}
\end{frame}
